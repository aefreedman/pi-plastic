...
| Create branch | `plastic_branchCreate(branch="/main/feature-name")` | `cm branch create /main/feature-name` | Hierarchical names |
| Switch branch | `plastic_switchBranch(branch="/main/feature-name")` | `cm switch --silent --noinput /main/feature-name` | `pendingChanges="shelve"` for tracked pending changes; private-only pending changes switch directly unattended |
